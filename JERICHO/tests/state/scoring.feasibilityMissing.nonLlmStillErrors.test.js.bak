  ...
  return {
    ...
    cyclesById: {
      'cycle-1': {
        ...
        planGenerationSource: 'GENERIC_DETERMINISTIC',
        metrics: {},
      },
    },
...
describe('scoring feasibility missing non-LLM path', () => {
  it('still emits FEASIBILITY_MISSING_FOR_PLAN for non-LLM source', () => {
    const next = computeDerivedState(buildState(), { type: 'NO_OP' });
    expect(next.lastPlanError?.code).toBe('FEASIBILITY_MISSING_FOR_PLAN');
  });
});
