...
describe('archetypeRuleQuality action coherence', () => {
  it('keeps action-to-deliverable mapping coherent', () => {
    migratedArchetypeFixtures.forEach((fixture) => {
      ...
      expect(summary.schedulerReadiness.actionDerivationCoherent).toBe(true);
    });
  });
});
