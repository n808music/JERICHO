...
describe('scoreGoalSuccessProbability', () => {
  it('returns capped value when evidence is insufficient', () => {
    ...
    const result = scoreGoalSuccessProbability(
      GOAL_ID,
      state,
      { timezone: 'UTC', maxBlocksPerDay: 2, scoringWindowDays: 3 },
      NOW_ISO
    );
  ...
  it('returns 0 when infeasible', () => {
    ...
    const result = scoreGoalSuccessProbability(
      GOAL_ID,
      state,
      { timezone: 'UTC', maxBlocksPerDay: 0, scoringWindowDays: 3 },
      NOW_ISO
    );
  ...
  it('handles sigma=0 deterministic outcomes with sufficient evidence', () => {
    ...
    const win = scoreGoalSuccessProbability(
      GOAL_ID,
      state,
      { timezone: 'UTC', maxBlocksPerDay: 2, scoringWindowDays: 7 },
      NOW_ISO
    );
    ...
    const lose = scoreGoalSuccessProbability(
      GOAL_ID,
      loseState,
      { timezone: 'UTC', maxBlocksPerDay: 2, scoringWindowDays: 7 },
      NOW_ISO
  ...
  it('counts completed evidence from the active cycle even when event goal ids lag the current contract goal id', () => {
    const state = makeState({
    ...
    expect(result.evidenceSummary?.completedCount).toBeGreaterThan(0);
    expect(result.status).not.toBe('NO_EVIDENCE');
  });

  it('counts historical completed create events as evidence', () => {
    const state = makeState({
      activeCycleId: 'cycle-1',
      executionEvents: [
        {
