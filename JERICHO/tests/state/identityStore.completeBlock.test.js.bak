...
describe('identityStore COMPLETE_BLOCK', () => {
  it('marks status completed across projections and appends ledger once (finite, clamped minutes)', () => {
    const state1 = identityReducer(makeStateWithBlockInAllProjections(makeBlock({ id: 'b1' })), {
      type: 'COMPLETE_BLOCK',
      id: 'b1',
    });
    ...
    if (weekBlock) {
      expect(weekBlock.status).toBe('completed');
    }
    const cycleBlock = (state1.cycle || []).flatMap((d) => d.blocks || []).find((b) => b.id === 'b1');
    if (cycleBlock) {
      expect(cycleBlock.status).toBe('completed');
    }
    ...
    const transitionTraceLog = (state1.debug?.traceLog || []).filter((entry) => entry?.transition);
