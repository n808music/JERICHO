...
describe('Elicitation Engine — read-back step', () => {
  it('halts at readback when all project slot gates pass (does not dispatch or finish)', () => {
    ...
    expect(step.readback.sentence).toMatch(/When this is done, you'll open/);
    ...
    expect(step.readback.fields.description).toBe('10,000 first-week streams');
  ...
  it('confirmed:true dispatches DECLARE_PROJECT and the engine is done', () => {
    ...
    expect(declare.payload.description).toBe('10,000 first-week streams');
  ...
  it('confirmed:false+reopen clears only the named field and re-probes it, preserving siblings', () => {
    const { engine } = driveToReadback(buildSeededState());
    const result = engine.confirmReadback({ confirmed: false, reopen: 'description' });
    ...
    expect(step.probe.fieldName).toBe('description');
  ...
  it('re-answering after reject re-emits readback with updated sentence, then confirms cleanly', () => {
    ...
    const rejected = engine.confirmReadback({ confirmed: false, reopen: 'description' });

    // Re-answer with a better deliverable
    const result2 = rejected.engine.consumeAnswer({ description: '500 tickets sold' });
    ...
    expect(decl.payload.description).toBe('500 tickets sold');
