...
import { ensureQuotaCriteria } from './deliverables.ts';
...
import { resolveActiveGoalContext } from './goalContext.js';
...
import { buildDraftScheduleItems, getDraftDiagnostics, isAutomationScheduleItem } from './draftSchedule.js';
import { DEFAULT_BOUNDARY_MODE, MAX_BLOCKS_PER_DAY } from './plannerConfig.ts';
import { classifyGoalToActionTemplate, ensureActionsForCycle } from '../domain/actions/actionTemplates.ts';
import {
  buildUnblockAction,
  computeReadyActions,
  getActionBlockers,
  hasUnblockAction,
  validateActionGraph,
} from '../domain/actions/actionSelectors.ts';
...
export function computeDerivedState(state, action) {
  ...
  switch (action.type) {
    ...
    case 'GENERATE_CYCLE_ACTIONS':
      generateCycleActions(next, action.payload);
      break;
    case 'SET_VIEW_DATE':
    ...
    case 'COMPLETE_ONBOARDING':
      ...
      next.meta = {
        ...
        scenarioLabel: action.onboarding?.scenarioLabel || next.meta?.scenarioLabel || '',
      };
      break;
    ...
    case 'SET_CALIBRATION_DAYS':
      ...
      break;
    case 'GENERATE_PLAN':
      if (blockSchedulingOnInvalidActionGraph(next, next.activeCycleId, action.type)) {
        break;
      }
      generatePlan(next);
      break;
    case 'APPLY_PLAN':
      ...
      break;
    case 'CREATE_UNBLOCK_ACTION':
      createUnblockAction(next, action.payload);
      break;
    case 'APPLY_DRAFT_SCHEDULE':
      ...
      break;
    case 'APPLY_DRAFT_SCHEDULE_FULL':
      if (blockSchedulingOnInvalidActionGraph(next, next.activeCycleId, action.type)) {
        break;
      }
    ...
    case 'COMMIT_PREVIEW_ITEMS': {
      ...
      if (blockSchedulingOnInvalidActionGraph(next, cycleId, action.type)) {
        break;
      }
      ...
      for (let index = 0; index < items.length; index += 1) {
        ...
        const requiresActionContext = isAutomationScheduleItem(item);
        if (requiresActionContext && (!item?.actionId || !item?.title || !item?.detail)) {
          next.lastPlanError = {
            code: 'MISSING_ACTION_CONTEXT',
            reason: `Automated preview item ${index + 1} missing action context.`,
          };
          if (process.env.NODE_ENV === 'development') {
            throw new Error(
              `Automated schedule item missing action context at index ${index + 1} (source=${item?.source || 'unknown'})`
            );
          }
          break;
        }
      ...
      for (let index = 0; index < items.length; index += 1) {
        ...
        createBlock(next, {
          ...
          title,
          detail: item.detail || '',
          actionId: item.actionId || item?.payload?.actionId || null,
        ...
        if (process.env.NODE_ENV !== 'production') {
          const event = next.executionEvents?.[next.executionEvents.length - 1];
          if (event?.kind === 'create') {
            console.log('preview_commit created event', event);
          }
        }
      }
    ...
    case 'REBUILD_SCHEDULE': {
      ...
      if (!cycle) {
        next.lastPlanError = {
          code: 'CYCLE_NOT_FOUND',
          reason: 'Active cycle missing for rebuild.',
        };
        break;
      }
      generateColdPlanForCycle(next, { rebaseMode: 'NONE' });
      rebuildDraftScheduleCache(next, cycleId);
      updateGoalAchievementStatus(next, cycleId);
      next.lastPlanError = null;
      break;
    }
    case 'INVALIDATE_FORECAST': {
      invalidateForecast(next, action.payload);
      break;
    }
    case 'PURGE_CONTEXTLESS_FORECAST_BLOCKS': {
      purgeContextlessForecastBlocks(next, action.payload);
      break;
    }
    case 'SET_STRATEGY':
  ...
  syncCriteriaCompletionFromEvents(next);
  const shouldMergePriorTodayBlocks = action.type !== 'PURGE_CONTEXTLESS_FORECAST_BLOCKS';
  if (shouldMergePriorTodayBlocks) {
...
export function hydrateActiveCycleState(state) {
  ...
  const cycleActions = cycle.actions || state.actionsByCycleId?.[cycle.id]?.actions || [];
  state.actionsByCycleId[cycle.id] = {
    cycleId: cycle.id,
    goalId: cycle.goalContract?.goalId || state.goalExecutionContract?.goalId || null,
    actions: cycleActions,
  };
  if (!cycleActions.length) {
    syncCycleActions(state);
  }
  return state;
}
...
export function persistActiveCycleState(state) {
  ...
  cycle.actions = state.actionsByCycleId?.[cycle.id]?.actions || cycle.actions || [];
...
function ensureDeliverablesStore(state) {
  ...
  if (!state.actionsByCycleId) state.actionsByCycleId = {};
  if (!state.draftScheduleItemsByCycleId) state.draftScheduleItemsByCycleId = {};
  if (!state.draftScheduleDiagnosticsByCycleId) state.draftScheduleDiagnosticsByCycleId = {};
}

function ensureActionStores(state) {
  if (!state.actionsByCycleId) state.actionsByCycleId = {};
}

function normalizeActionStatusFromBlocks(blocks = []) {
  if (!blocks.length) return 'todo';
  const statuses = blocks.map((block) => (block?.status || '').toString().toLowerCase());
  if (statuses.some((status) => status === 'completed' || status === 'done')) return 'done';
  if (statuses.every((status) => status === 'expired' || status === 'skipped')) return 'skipped';
  if (statuses.some((status) => status === 'in_progress' || status === 'active')) return 'in_progress';
  return 'todo';
}

function sortGoalActions(actions = []) {
...
function getCycleRequestedRouteSlots(cycle) {
  const forecastByDayKey = cycle?.coldPlan?.forecastByDayKey || {};
  return Object.values(forecastByDayKey).reduce((sum, entry) => {
    const totalBlocks = Number(entry?.totalBlocks || 0);
    return sum + (Number.isFinite(totalBlocks) ? totalBlocks : 0);
  }, 0);
}

function resolveGoalTextForTemplate(goalContext, goal) {
  return (
    goalContext?.goalText ||
    goal?.terminalOutcome?.text ||
    goal?.goalText ||
    goal?.goalLabel ||
    goal?.label ||
    goal?.title ||
...
function isShowWritingCycle(goalContext, goal) {
  return classifyGoalToActionTemplate(resolveGoalTextForTemplate(goalContext, goal)) === 'show_writing';
}

function syncCycleActions(state) {
  ensureActionStores(state);
  const cycleId = state.activeCycleId;
  const cycle = cycleId ? state.cyclesById?.[cycleId] : null;
  if (!cycle || !cycleId) return;
  const goalContext = resolveActiveGoalContext(state, cycleId);
  const goal = goalContext?.goal || cycle.goalContract || state.goalExecutionContract || null;
  const goalId = goalContext?.goalId || cycle.goalGovernanceContract?.goalId || state.activeGoalId;
  if (!goalId || !goalContext?.goalText) return;

  const showWriting = isShowWritingCycle(goalContext, goal);
  const existingWorkspace = state.actionsByCycleId[cycleId];
  const existingActions = existingWorkspace?.goalId === goalId ? existingWorkspace.actions || [] : [];
  const previousActionCount = existingActions.length;
  const generated = ensureActionsForCycle({
    cycleId,
    goal,
    goalId,
    existingActions: showWriting ? [] : existingActions,
  });

  const byActionId = new Map();
  const previousStatusByActionId = new Map(
    existingActions.filter((action) => action?.id).map((action) => [action.id, action.status || 'todo'])
  );
  getAllBlocks(state)
    .filter((block) => block?.cycleId === cycleId && block?.actionId)
    .forEach((block) => {
      const bucket = byActionId.get(block.actionId) || [];
      bucket.push(block);
      byActionId.set(block.actionId, bucket);
    });
  const completedActionIds = new Set(
    (state.executionEvents || [])
      .filter((event) => event?.cycleId === cycleId && event?.kind === 'complete' && event?.actionId)
      .map((event) => event.actionId)
  );

  const synced = generated
    .map((action) => {
      const linkedBlocks = byActionId.get(action.id) || [];
      const previousStatus = previousStatusByActionId.get(action.id);
      const status = completedActionIds.has(action.id)
        ? 'done'
        : previousStatus === 'done'
          ? 'done'
          : normalizeActionStatusFromBlocks(linkedBlocks);
  ...
  const validation = applyActionGraphValidation(state, cycleId, synced);
  if (!validation.valid) {
    return;
  }

  state.actionsByCycleId[cycleId] = { cycleId, goalId, actions: synced };
  cycle.actions = synced;
  state.cyclesById[cycleId] = cycle;
  autoHealForecastAfterActionAvailability(state, {
    cycleId,
    previousActionCount,
    nextActionCount: synced.length,
    force: false,
  });
}

function generateCycleActions(state, payload = {}) {
  ensureActionStores(state);
  ...
  const goal = goalContext?.goal || cycle.goalContract || state.goalExecutionContract || null;
  const goalId = goalContext?.goalId || cycle.goalGovernanceContract?.goalId || state.activeGoalId;
  if (!goal || !goalId || !goalContext?.goalText) return;
  const showWriting = isShowWritingCycle(goalContext, goal);
  const previousActionCount = (state.actionsByCycleId?.[cycleId]?.actions || cycle.actions || []).length;
  const existing = showWriting
    ? []
    : payload?.force
      ? []
      : state.actionsByCycleId?.[cycleId]?.actions || cycle.actions || [];
  const nextActions = ensureActionsForCycle({ cycleId, goal, goalId, existingActions: existing });

  const validation = applyActionGraphValidation(state, cycleId, nextActions);
  if (!validation.valid) {
    return;
  }

  state.actionsByCycleId[cycleId] = { cycleId, goalId, actions: nextActions };
...
function applyActionGraphValidation(state, cycleId, actions = []) {
  const validation = validateActionGraph(actions);
  if (!validation.valid) {
    const reasons = validation.errors.map((error) => `Action graph integrity error: ${error}`);
    state.lastPlanError = {
      code: 'ACTION_GRAPH_INVALID',
      reason: reasons[0] || 'Action dependency graph is invalid.',
      reasons,
      cycleId,
    };
    if (process.env.NODE_ENV !== 'production') {
      console.warn('ACTION_GRAPH_INVALID', {
        cycleId,
  ...
  if (state.lastPlanError?.code === 'ACTION_GRAPH_INVALID') {
    state.lastPlanError = null;
  }
  return validation;
}

function getCycleActionsForValidation(state, cycleId) {
  if (!cycleId) return [];
  const workspace = state.actionsByCycleId?.[cycleId]?.actions || [];
  if (workspace.length) return workspace;
  return state.cyclesById?.[cycleId]?.actions || [];
}

function getInvalidActionGraphDetails(state, cycleId) {
  const actions = getCycleActionsForValidation(state, cycleId);
...
function blockSchedulingOnInvalidActionGraph(state, cycleId, actionType) {
  const details = getInvalidActionGraphDetails(state, cycleId);
  if (!details) return false;
  const reasons = details.errors.map((error) => `Action graph integrity error: ${error}`);
  state.lastPlanError = {
    code: 'ACTION_GRAPH_INVALID',
    reason: reasons[0] || `Action graph invalid during ${actionType}.`,
    reasons,
    cycleId: details.cycleId,
    actionType,
  };
  if (process.env.NODE_ENV !== 'production') {
    console.warn('ACTION_GRAPH_INVALID gate', {
      cycleId: details.cycleId,
      actionType,
...
function createUnblockAction(state, payload = {}) {
  const cycleId = payload?.cycleId || state.activeCycleId;
  if (!cycleId) {
    state.lastPlanError = {
      code: 'UNBLOCK_NOT_ALLOWED',
      reason: 'Cannot create unblock action without an active cycle.',
    };
    return;
  }
  if (blockSchedulingOnInvalidActionGraph(state, cycleId, 'CREATE_UNBLOCK_ACTION')) {
    return;
  }
  ...
  if (!cycle) {
    state.lastPlanError = {
      code: 'UNBLOCK_NOT_ALLOWED',
      reason: 'Cannot create unblock action: cycle not found.',
    };
    return;
  }

  const workspace = state.actionsByCycleId?.[cycleId];
  const actions = workspace?.actions?.length ? workspace.actions : cycle.actions || [];
  const hasActionPlan = actions.length > 0;
  const readyActions = computeReadyActions(actions);
  const routeSlots = getCycleRequestedRouteSlots(cycle);

  if (!hasActionPlan || readyActions.length > 0 || routeSlots <= 0) {
    state.lastPlanError = {
      code: 'UNBLOCK_NOT_ALLOWED',
      reason: !hasActionPlan
        ? 'Cannot create unblock action without an action plan.'
        : readyActions.length > 0
          ? 'Unblock action is only allowed when no actions are dependency-ready.'
          : 'Unblock action is only allowed when forecast route slots exist.',
      cycleId,
    };
    return;
  }

  if (hasUnblockAction(actions)) {
    state.lastPlanError = {
      code: 'UNBLOCK_ALREADY_EXISTS',
      reason: 'An unblock action already exists for this cycle.',
      cycleId,
    };
    return;
  }
  ...
  if (!blockers.length) {
    state.lastPlanError = {
      code: 'UNBLOCK_NO_BLOCKERS',
      reason: 'No blockers found; unblock action was not created.',
      cycleId,
    };
    return;
  }
  ...
  const goalId =
    workspace?.goalId || cycle.goalContract?.goalId || state.goalExecutionContract?.goalId || state.activeGoalId;
  if (!goalId) {
    state.lastPlanError = {
      code: 'UNBLOCK_NOT_ALLOWED',
      reason: 'Cannot create unblock action without a goal ID.',
      cycleId,
    };
    return;
  }

  const unblockAction = buildUnblockAction({ cycleId, goalId, blockers });
  const nextActions = sortGoalActions([...actions, unblockAction]);
  state.actionsByCycleId[cycleId] = { cycleId, goalId, actions: nextActions };
  cycle.actions = nextActions;
  state.cyclesById[cycleId] = cycle;
  applyActionGraphValidation(state, cycleId, nextActions);
  if (state.lastPlanError?.code === 'ACTION_GRAPH_INVALID') {
    return;
  }
  invalidateForecast(state, { cycleId });
...
function autoHealForecastAfterActionAvailability(
  state,
  { cycleId, previousActionCount = 0, nextActionCount = 0, force = false } = {}
) {
  if (!cycleId || nextActionCount <= 0) return;
  const transitionedToActionPlan = previousActionCount <= 0 && nextActionCount > 0;
  if (!transitionedToActionPlan && !force) return;
  const cycle = state.cyclesById?.[cycleId];
  if (!cycle) return;

  const hasForecastArtifacts =
    Object.keys(cycle?.coldPlan?.forecastByDayKey || {}).length > 0 ||
    (cycle?.suggestedBlocks || []).length > 0 ||
    (state.activeCycleId === cycleId && (state.suggestedBlocks || []).length > 0);
  const hasContextlessDraftBlocks = getAllBlocks(state).some((block) => {
    if (!block || block.cycleId !== cycleId) return false;
    const status = (block.status || '').toString().toLowerCase();
    const complete = status === 'completed' || status === 'done';
    return !complete && block.origin === 'draft' && !block.actionId;
  });
  if (!hasForecastArtifacts && !hasContextlessDraftBlocks) return;

  invalidateForecast(state, { cycleId });
  if (hasContextlessDraftBlocks) {
    purgeContextlessDraftBlocksForCycle(state, { cycleId });
  }
  if (state.activeCycleId === cycleId) {
    generateColdPlanForCycle(state, { rebaseMode: 'NONE' });
  }
...
function updateGoalAchievementStatus(state, cycleId) {
  ...
  const allCriteria = deliverables.flatMap((d) => d.criteria || []);
  if (!allCriteria.length) return;
  const allDone = allCriteria.every((c) => c?.isDone);
  const nowISO = state.appTime?.nowISO || new Date().toISOString();
  const cycle = state.cyclesById[cycleId];
  cycle.goalCompletion = cycle.goalCompletion || { status: 'INCOMPLETE' };
  if (allDone) {
    if (cycle.goalCompletion.status !== 'ACHIEVED') {
      cycle.goalCompletion = {
        status: 'ACHIEVED',
        achievedAtISO: nowISO,
      };
    }
  } else if (cycle.goalCompletion.status !== 'INCOMPLETE') {
    cycle.goalCompletion = {
      status: 'INCOMPLETE',
      achievedAtISO: null,
    };
...
function syncCriteriaCompletionFromEvents(state) {
  const deliverablesByCycleId = state.deliverablesByCycleId || {};
  const allBlocks = getAllBlocks(state);
  const completedByCriterion = new Set(
    allBlocks
      .filter((block) => block?.status === 'completed' && block?.deliverableId && block?.criterionId)
      .map((block) => block.criterionId)
  );
  Object.keys(deliverablesByCycleId || {}).forEach((cycleId) => {
    const workspace = deliverablesByCycleId[cycleId];
    if (!workspace?.deliverables?.length) return;
    workspace.deliverables = workspace.deliverables.map((deliverable) => {
      const nextCriteria = (deliverable.criteria || []).map((criterion) => {
        const isDone = completedByCriterion.has(criterion.id);
        if (criterion.isDone === isDone) return criterion;
        return {
          ...criterion,
          isDone,
          doneAtISO: isDone ? criterion.doneAtISO || state.appTime?.nowISO || new Date().toISOString() : null,
          doneAtDayKey: isDone ? criterion.doneAtDayKey || state.appTime?.activeDayKey || nowDayKey() : null,
        };
      });
      return { ...deliverable, criteria: nextCriteria };
    });
...
function generateColdPlanForCycle(state, { rebaseMode = 'NONE' } = {}) {
  ...
  const contractStartRaw =
    cycle.goalContract?.temporalBinding?.startDayKey ||
    cycle.goalContract?.startDayKey ||
    cycle.goalContract?.startDateISO ||
    cycle.goalContract?.startDate ||
    cycle.goalContract?.startISO ||
    null;
  const resolvedContractStart =
    typeof contractStartRaw === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(contractStartRaw)
      ? contractStartRaw
      : contractStartRaw
        ? dayKeyFromISO(contractStartRaw, timeZone)
        : null;
  const startDayKey = resolvedContractStart || cycle.startedAtDayKey || dayKeyFromISO(nowISO, timeZone);
  ...
  if (deliverables.length === 0 || totalRequired === 0) {
    ...
    if (workspace?.deliverables?.length > 0) {
      ...
      deliverables = normalizeDeliverables(ensureQuotaCriteria(workspace.deliverables, nowISO));
      ...
      try {
        deliverables = ensureQuotaCriteria(generateAutoDeliverables(cycle.goalContract) || [], nowISO);
        totalRequired = deliverables.reduce((sum, d) => sum + d.requiredBlocks, 0);
      ...
      if (!deliverables || deliverables.length === 0) {
        ...
        deliverables = ensureQuotaCriteria(autoResult.deliverables || [], nowISO);
        totalRequired = deliverables.reduce((sum, d) => sum + d.requiredBlocks, 0);
  ...
  if (mechanismClass === 'GENERIC_DETERMINISTIC') {
    ...
    const deterministicResult = generateDeterministicPlan({
      ...
      nowDayKey,
      causalChainSteps: cycle.goalContract?.causalChain?.steps,
      constraints: {
...
function deleteCycle(state, cycleId) {
  ...
  if (state.activeCycleId === cycleId) {
    ...
    state.executionEvents = [];
    state.suggestionHistory = {
      dayKey: state.appTime?.activeDayKey || nowDayKey(),
      count: 0,
      lastSuggestedAtISO: null,
      lastSuggestedAtISOByGoal: {},
      dailyCountByGoal: {},
...
function endCycle(state, cycleId) {
  ...
  const deliverables =
    state.deliverablesByCycleId?.[cycle.id]?.deliverables || cycle?.deliverables || cycle?.strategy?.deliverables || [];
...
function rebuildDraftScheduleCache(state, cycleIdOverride = null) {
  const cycleId = cycleIdOverride || state.activeCycleId;
  const cycle = cycleId ? state.cyclesById?.[cycleId] : getActiveCycle(state);
  const contract = cycle?.goalContract || state.goalExecutionContract;
  if (!cycle || !contract || !cycleId) return [];
  if (!state.draftScheduleItemsByCycleId) state.draftScheduleItemsByCycleId = {};
  const workspace = getDeliverableWorkspace(state, cycle.id);
  const deliverables = ensureQuotaCriteria(
  ...
  const timeZone = state.appTime?.timeZone || 'UTC';
  const startDayKeyForDraft = state.appTime?.activeDayKey || state.today?.date || nowDayKey(timeZone);
  const goalDeadlineDayKeyForDraft = cycle?.goalContract?.deadline?.dayKey || null;
  const inferredFullPlanHorizonDays = (() => {
    if (!startDayKeyForDraft || !goalDeadlineDayKeyForDraft) return null;
    const startAt = Date.parse(`${startDayKeyForDraft}T00:00:00.000Z`);
    const endAt = Date.parse(`${goalDeadlineDayKeyForDraft}T00:00:00.000Z`);
    if (!Number.isFinite(startAt) || !Number.isFinite(endAt) || endAt < startAt) return null;
    return Math.floor((endAt - startAt) / 86400000) + 1;
  })();
  let fullPlanStats = null;
  const draftItems = buildDraftScheduleItems(state, cycle.id, {
    startDateISO: startDayKeyForDraft,
    boundaryMode: DEFAULT_BOUNDARY_MODE,
    suggestedBlocks,
    routeSuggestions,
    deliverables,
    actions: state.actionsByCycleId?.[cycle.id]?.actions || cycle.actions || [],
    contract,
    timeZone,
    defaults: {
      todayKey: startDayKeyForDraft,
      primaryDomain: contract?.primaryDomain || 'FOCUS',
      routeMinutes: state.planDraft?.routeMinutes || 30,
      executionHorizonDays: state.planDraft?.executionHorizonDays || 90,
      fullPlanMaxHorizonDays: state.planDraft?.fullPlanMaxHorizonDays || inferredFullPlanHorizonDays || null,
    },
    captureStats: (stats) => {
      fullPlanStats = stats || null;
    },
  });
  const criteriaQueueByDeliverable = new Map();
  deliverables.forEach((deliverable) => {
    if (!deliverable?.id) return;
    const remaining = (deliverable.criteria || []).filter((c) => !c?.isDone).map((c) => c.id);
    criteriaQueueByDeliverable.set(deliverable.id, remaining);
  });
  draftItems.forEach((item) => {
    const deliverableId = item?.payload?.deliverableId || item?.deliverableId || null;
    if (!deliverableId) return;
    if (item?.payload?.criterionId) return;
    const queue = criteriaQueueByDeliverable.get(deliverableId) || [];
    const nextCriterionId = queue.shift();
    if (!nextCriterionId) return;
    criteriaQueueByDeliverable.set(deliverableId, queue);
    item.payload = { ...(item.payload || {}), deliverableId, criterionId: nextCriterionId };
  });
  const goalDeadlineDayKey = goalDeadlineDayKeyForDraft;
  const diagnostics = getDraftDiagnostics({
    state,
    cycleId: cycle.id,
    actions: state.actionsByCycleId?.[cycle.id]?.actions || cycle.actions || [],
    draftItems,
    fullDraftItems: draftItems,
    scheduleMode: 'FULL_PLAN',
    requiredSlotsWeighted: fullPlanStats?.requiredSlotsWeighted ?? null,
    availableSlotsFromPlacement: fullPlanStats?.availableSlots ?? null,
    missingSlotsWeighted: fullPlanStats?.missingSlotsWeighted ?? null,
    unusedSlots: fullPlanStats?.unusedSlots ?? null,
    unplacedEstimateMinTotal: fullPlanStats?.unplacedEstimateMinTotal ?? null,
    unplacedEstimateMinByCategory: fullPlanStats?.unplacedEstimateMinByCategory ?? null,
    milestonePrioritySummary: fullPlanStats?.milestonePrioritySummary ?? null,
    unassignedActionReasons: fullPlanStats?.unassignedActionReasons ?? null,
    milestoneWindowMissCountPlacement: fullPlanStats?.milestoneWindowMissCountPlacement ?? null,
    depWindowBlockedCount: fullPlanStats?.depWindowBlockedCount ?? null,
    depWindowBlockedByMilestone: fullPlanStats?.depWindowBlockedByMilestone ?? null,
    depBufferBlockedCount: fullPlanStats?.depBufferBlockedCount ?? null,
    depBufferBlockedByMilestone: fullPlanStats?.depBufferBlockedByMilestone ?? null,
    reservationInputCount: fullPlanStats?.reservationInputCount ?? null,
    reservationAcceptedPass1Count: fullPlanStats?.reservationAcceptedPass1Count ?? null,
    reservationPendingDepValidationCount: fullPlanStats?.reservationPendingDepValidationCount ?? null,
    reservationRejectedCount: fullPlanStats?.reservationRejectedCount ?? null,
    orphanReservationCount: fullPlanStats?.orphanReservationCount ?? null,
    preservedChunkCount: fullPlanStats?.preservedChunkCount ?? null,
    movedChunkCount: fullPlanStats?.movedChunkCount ?? null,
    droppedChunkCount: fullPlanStats?.droppedChunkCount ?? null,
    churnMovedMinutesTotal: fullPlanStats?.churnMovedMinutesTotal ?? null,
    churnReasonsCount: fullPlanStats?.churnReasonsCount ?? null,
    rescheduleDecisions: fullPlanStats?.rescheduleDecisions ?? null,
    outsideExecutionHorizonCount: fullPlanStats?.outsideExecutionHorizonCount ?? null,
    outsideExecutionHorizonEstimateMinTotal: fullPlanStats?.outsideExecutionHorizonEstimateMinTotal ?? null,
    executionHorizonDays: fullPlanStats?.executionHorizonDays ?? null,
    executionWindowStartISO: fullPlanStats?.executionWindowStartISO ?? null,
    executionWindowEndISO: fullPlanStats?.executionWindowEndISO ?? null,
    feasibilityWindowStartISO: fullPlanStats?.feasibilityWindowStartISO ?? null,
    feasibilityWindowEndISO: fullPlanStats?.feasibilityWindowEndISO ?? null,
    slotUniverseMode: fullPlanStats?.slotUniverseMode ?? null,
    slotUniverseCandidateCount: fullPlanStats?.slotUniverseCandidateCount ?? null,
    slotUniverseSelectedCount: fullPlanStats?.slotUniverseSelectedCount ?? null,
    placementHorizonEndISO: fullPlanStats?.placementHorizonEndISO ?? null,
    placementHorizonRequestedEndISO: fullPlanStats?.placementHorizonRequestedEndISO ?? null,
    placementHorizonGuardedEndISO: fullPlanStats?.placementHorizonGuardedEndISO ?? null,
    placementHorizonGuardApplied: fullPlanStats?.placementHorizonGuardApplied ?? null,
    placementHorizonGuardDays: fullPlanStats?.placementHorizonGuardDays ?? null,
    placementHorizonDays: fullPlanStats?.placementHorizonDays ?? null,
    placementWindowStartISO: fullPlanStats?.placementWindowStartISO ?? null,
    placementWindowEndISO: fullPlanStats?.placementWindowEndISO ?? null,
    milestoneWindowConstraintMode: fullPlanStats?.milestoneWindowConstraintMode ?? null,
    horizonMode: fullPlanStats?.horizonMode ?? null,
    softWindowFallbackCount: fullPlanStats?.softWindowFallbackCount ?? null,
    qualityScoreBaseline: fullPlanStats?.qualityScoreBaseline ?? null,
    qualityScoreOptimized: fullPlanStats?.qualityScoreOptimized ?? null,
    qualityImprovementDelta: fullPlanStats?.qualityImprovementDelta ?? null,
    chosenMovesSummary: fullPlanStats?.chosenMovesSummary ?? null,
    reasonCodeOverride: fullPlanStats?.reasonCode ?? null,
    startDateISO: startDayKeyForDraft,
    deadlineISO: goalDeadlineDayKey ? `${goalDeadlineDayKey}T23:59:59.000Z` : null,
    maxBlocksPerDay: MAX_BLOCKS_PER_DAY,
  });
  state.draftScheduleItemsByCycleId[cycle.id] = draftItems;
  state.draftScheduleDiagnosticsByCycleId[cycle.id] = diagnostics;
  return draftItems;
}

function commitDraftItems(state, cycleId, items = []) {
  const cycle = cycleId ? state.cyclesById?.[cycleId] : null;
  if (!cycle) {
    state.lastPlanError = {
      code: 'CYCLE_NOT_FOUND',
  ...
  const goalId = cycle.goalContract?.goalId || state.goalExecutionContract?.goalId;
  for (let index = 0; index < items.length; index += 1) {
    const item = items[index];
    const requiresActionContext = isAutomationScheduleItem(item);
    if (requiresActionContext && (!item?.actionId || !item?.title || !item?.detail)) {
  ...
  items.forEach((item, index) => {
    ...
    createBlock(state, {
      ...
      status: item.state || item?.payload?.state || null,
      title,
      detail: item.detail || '',
      category: item.category || null,
      actionId: item.actionId || item?.payload?.actionId || null,
      ...
      blocked: item?.meta?.blocked ?? item?.payload?.blocked ?? item?.blocked ?? false,
      completedMarker: item?.meta?.completedMarker ?? item?.payload?.completedMarker ?? item?.completedMarker ?? false,
      unmetDepIds: item?.meta?.unmetDepIds || item?.payload?.unmetDepIds || [],
      unmetDepTitles: item?.meta?.unmetDepTitles || item?.payload?.unmetDepTitles || [],
      blockReason: item?.meta?.blockReason || item?.payload?.blockReason || null,
      estimateMin: item?.meta?.estimateMin ?? item?.payload?.estimateMin ?? item?.estimateMin ?? null,
      allocatedMin: item?.meta?.allocatedMin ?? item?.payload?.allocatedMin ?? item?.allocatedMin ?? duration,
      chunkIndex: item?.meta?.chunkIndex ?? item?.payload?.chunkIndex ?? item?.chunkIndex ?? 0,
      chunkCount: item?.meta?.chunkCount ?? item?.payload?.chunkCount ?? item?.chunkCount ?? 1,
      meta: item?.meta || null,
      origin: 'preview_commit',
      surface: 'today',
    });
...
function applyDraftSchedule(state, payload = {}) {
  const cycle = getActiveCycle(state);
  if (!cycle) return;
  const cycleId = payload?.cycleId || cycle.id;
  const cachedItems = state?.draftScheduleItemsByCycleId?.[cycleId];
  const cachedDiagnostics = state?.draftScheduleDiagnosticsByCycleId?.[cycleId] || null;
  const draftItems = Array.isArray(payload?.items)
    ? payload.items
    : Array.isArray(cachedItems) && cachedItems.length
      ? cachedItems
      : rebuildDraftScheduleCache(state, cycleId);
  const blockReasonCode = cachedDiagnostics?.reasonCode;
  if (blockReasonCode === 'INSUFFICIENT_ROUTE_SLOTS_TO_DEADLINE' || blockReasonCode === 'NO_GOAL_DEADLINE') {
    state.lastPlanError = {
      code: blockReasonCode,
      reason:
        blockReasonCode === 'NO_GOAL_DEADLINE'
          ? 'Cannot schedule: goal deadline is missing.'
          : 'Cannot schedule full plan: not enough route slots before deadline.',
      details: {
        requiredSlots: cachedDiagnostics?.requiredSlots ?? 0,
        availableSlots: cachedDiagnostics?.availableSlots ?? 0,
        missingSlots: cachedDiagnostics?.missingSlots ?? 0,
        startDateISO: cachedDiagnostics?.startDateISO || null,
        deadlineISO: cachedDiagnostics?.deadlineISO || null,
        maxBlocksPerDay: cachedDiagnostics?.maxBlocksPerDay ?? MAX_BLOCKS_PER_DAY,
      },
    };
    return;
  }
  if (!commitDraftItems(state, cycleId, draftItems)) return;
...
function invalidateForecast(state, payload = {}) {
  const cycleId = payload?.cycleId || state.activeCycleId;
  const cycle = cycleId ? state.cyclesById?.[cycleId] : null;
  if (!cycle) return;
  cycle.coldPlan = { forecastByDayKey: {}, dailyProjection: { forecastByDayKey: {} } };
  ...
  if (state.draftScheduleItemsByCycleId) state.draftScheduleItemsByCycleId[cycleId] = [];
  if (state.draftScheduleDiagnosticsByCycleId) delete state.draftScheduleDiagnosticsByCycleId[cycleId];
  state.cyclesById[cycleId] = cycle;
...
function purgeContextlessForecastBlocks(state, payload = {}) {
  const cycleId = payload?.cycleId || state.activeCycleId;
  if (!cycleId) return;
  const targetEvents = (state.executionEvents || []).filter(
    (event) =>
      event?.kind === 'create' &&
      event?.cycleId === cycleId &&
      event?.origin === 'preview_commit' &&
      !event?.actionId &&
      (event?.status === 'planned' || event?.status === 'scheduled')
  );
  targetEvents.forEach((event) => {
    if (!event?.blockId) return;
    const existing = findBlockById(state, event.blockId);
    if (existing) {
      deleteBlock(state, event.blockId);
      return;
    }
    const startISO = event.startISO || (event.dateISO ? `${event.dateISO}T09:00:00.000Z` : null);
    if (!startISO) return;
    const synthetic = {
      id: event.blockId,
      cycleId: event.cycleId,
      goalId: event.goalId,
      practice: event.domain || 'FOCUS',
      domain: event.domain || 'FOCUS',
      label: event.rawLabel || event.title || 'Block',
      title: event.title || event.rawLabel || 'Block',
      detail: event.detail || '',
      start: startISO,
      end: event.endISO || startISO,
      status: event.status || 'planned',
      origin: event.origin,
      actionId: event.actionId || null,
    };
    const deleteEvent = buildExecutionEventFromBlock(synthetic, {
      kind: 'delete',
      completed: false,
      minutes: 0,
    });
    if (!canEmitExecutionEvent(state.executionEvents || [], deleteEvent)) return;
    appendExecutionEvent(state, deleteEvent);
  });
}

function purgeContextlessDraftBlocksForCycle(state, payload = {}) {
  const cycleId = payload?.cycleId || state.activeCycleId;
  if (!cycleId) return;
  const targets = getAllBlocks(state).filter((block) => {
    if (!block || block.cycleId !== cycleId) return false;
    const status = (block.status || '').toString().toLowerCase();
    const complete = status === 'completed' || status === 'done';
    return !complete && block.origin === 'draft' && !block.actionId;
...
function compileGoalEquation(state, payload = {}) {
  ...
  if (workspace && (!workspace.deliverables || workspace.deliverables.length === 0)) {
    ...
    if (autoDeliverables && autoDeliverables.length > 0) {
      workspace.deliverables = ensureQuotaCriteria(autoDeliverables, nowISO);
      ...
      if (!cycle.strategy) {
        ...
        cycle.strategy.deliverables = workspace.deliverables;
        cycle.strategy.assumptionsHash = buildAssumptionsHash(cycle.strategy);
      }
    }
  }
...
function applyCalibrationDays(state, daysPerWeek, uncertain = false) {
  ...
  const prevSuggestionIds = (state.suggestedBlocks || []).filter((s) => s && s.status === 'suggested').map((s) => s.id);
...
function toggleCriterionDone(state, payload = {}) {
  ...
  syncDeliverableWorkspaceIndexes(workspace);
  state.deliverablesByCycleId[cycleId] = workspace;
  updateGoalAchievementStatus(state, cycleId);
}
...
function handleDraftBlockCreate(state, action = {}) {
  ...
  const detail = action.detail || action.description || '';
  const category = action.category || practice || domain;
...
function createBlock(state, payload = {}) {
  ...
  const title = payload.title || payload.label || practice || 'Block';
  const detail = payload.detail || payload.description || '';
  const category = payload.category || practice || domain;
  const actionId = payload.actionId || null;
  const blocked = Boolean(payload.blocked || payload?.meta?.blocked);
  const completedMarker = Boolean(payload.completedMarker || payload?.meta?.completedMarker);
  const unmetDepIds = Array.isArray(payload.unmetDepIds)
    ? payload.unmetDepIds
    : Array.isArray(payload?.meta?.unmetDepIds)
      ? payload.meta.unmetDepIds
      : [];
  const unmetDepTitles = Array.isArray(payload.unmetDepTitles)
    ? payload.unmetDepTitles
    : Array.isArray(payload?.meta?.unmetDepTitles)
      ? payload.meta.unmetDepTitles
      : [];
  const blockReason = payload.blockReason || payload?.meta?.blockReason || null;
...
function updateBlock(state, payload = {}) {
  ...
  const applyUpdate = (block) => {
    ...
    const title = payload.title || payload.label || block.title || block.label;
    return {
      ...block,
      practice,
      domain,
      title,
      detail: Object.prototype.hasOwnProperty.call(payload, 'detail') ? payload.detail : block.detail,
      category: payload.category || block.category || practice || domain,
      actionId: Object.prototype.hasOwnProperty.call(payload, 'actionId') ? payload.actionId : block.actionId,
      label: payload.label || title || block.label,
      start: startDate.toISOString(),
      end: endDate.toISOString(),
      status,
