...
export const normalizeSuggestionDayKey = (suggestion, timeZone) => {
  ...
  if (suggestion.dayKey) {
    return suggestion.dayKey;
  }
...
export const filterSuggestedBlocksByStartDate = (suggestedBlocks = [], startISO, timeZone) => {
  ...
  return (suggestedBlocks || []).filter((s) => {
    ...
    if (s?.startISO && isBeforeStartDate(s.startISO, startISO)) {
      return false;
    }
    if (s?.start && isBeforeStartDate(s.start, startISO)) {
      return false;
    }
    const dayKey = normalizeSuggestionDayKey(s, timeZone);
    if (dayKey && isBeforeStartDate(dayKey, startISO)) {
      return false;
    }
    return true;
  });
...
export const selectVisibleDraftItems = ({
  ...
  return (draftItems || [])
    .filter((item) => {
      ...
      if (startKey && item.dayKey < startKey) {
        return false;
      }
      if (endKey && item.dayKey > endKey) {
        return false;
      }
      return true;
    })
