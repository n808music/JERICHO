...
describe('Freeze: Cycle Isolation', () => {
  it('cycle A -> end/archive -> start cycle B isolates evidence and projections', () => {
    ...
    state = computeDerivedState(state, {
      ...
      payload: { cycleId: cycleA, title: 'A Task', requiredBlocks: 1 },
    });
    state = computeDerivedState(state, {
      type: 'CREATE_BLOCK',
      payload: {
        start: localStartISOForHour(10),
        durationMinutes: 30,
        domain: 'CREATION',
        title: 'A Block',
        timeZone: 'UTC',
        linkToGoal: true,
        deliverableId: state.deliverablesByCycleId?.[cycleA]?.deliverables?.[0]?.id,
      },
    });
    ...
    state = {
      ...
      cyclesById: { ...state.cyclesById, [cycleA]: { ...(state.cyclesById[cycleA] || {}), executionEvents: events } },
    };
    ...
    // Start cycle B
    const started = computeDerivedState(ended, {
      type: 'START_NEW_CYCLE',
      payload: { goalText: 'Cycle B goal', deadlineDayKey: '2026-03-08' },
    });
    ...
    expect(
      started.cyclesById[cycleA].executionEvents && started.cyclesById[cycleA].executionEvents.length
    ).toBeGreaterThan(0);
  });
});
