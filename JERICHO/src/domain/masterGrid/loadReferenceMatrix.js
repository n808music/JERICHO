...
export function loadReferenceMatrix(fixture, { nowISO = new Date().toISOString() } = {}) {
  ...
  // Resolve node names to IDs with appropriate prefixes per class (aligns with builder schemes).
  ...
  const getNodeId = (nm) => {
    ...
    switch (nodeClass) {
      case 'Entity': return `entity-${baseId}`;
      case 'Initiative': return `initiative-${baseId}`;
      case 'Project': return `project-${baseId}`;
      case 'Deliverable': return `deliverable-${baseId}`;
      case 'System': return `system-${baseId}`;
      case 'Artifact': return baseId; // Artifact builder uses bare slug
  ...
  // Entities use type-prefix scheme (entity-${slug})
      ...
      if (cls === 'Entity') {
        ...
        dispatch({
          ...
          payload: {
            ...common,
            owningEntityId: resolveEntity(n.owner),
            owningInitiativeId: getNodeId(n.parent_initiative),
        ...
        const owningProjectId = getNodeId(n.parent_project);
        ...
        // Deliverables use type-prefix scheme (deliverable-${slug})
