...
export function derivePlanQualityStandardAuditInput(state, { cycleId = null } = {}) {
...
export function runPlanQualityStandardComplianceAudit(input) {
  const context =
    input?.state && !input?.activeScheduledBlocks ? derivePlanQualityStandardAuditInput(input.state, { cycleId: input?.cycleId }) : input;
...
export function formatPlanQualityStandardAuditReport(report) {
  const lines = [
    '[Sample Enterprise] Standard Compliance Audit',
    '',
