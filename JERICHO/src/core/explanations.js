...
export const zionModeSpec = {
  ...
  components: {
    Orb: {
      props: "state: 'on-track'|'drifting'|'off-track', label?, delta?",
      variants: 'sm/md/lg; ring thickness = urgency',
    },
    ...
    TimeBlock: {
      props: '{ label, discipline, start, end, priority?, state }',
      visual: 'fill = discipline tint, accent stripe for priority',
    },
...
export const controlRoomCopilotSpec = {
  ...
  audio: 'Same reasoning path; output can be sent to TTS for narration of schedule or context—no extra logic layer.',
