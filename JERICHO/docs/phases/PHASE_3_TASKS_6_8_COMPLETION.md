...
Successfully implemented automatic deliverables seeding across the goal
admission → cold plan generation pipeline. The fix resolves the DEADLINE_INVALID
error that occurred in post-admission plan regenerations.

## Root Cause Fixed

**Problem**: Deliverables auto-generated at goal admission time were **not being
persisted** to the workspace, causing `generateColdPlanForCycle` to either:
...
**Key Fix**: When deliverables are found in workspace, now updates
...
**Key Feature**: Ensures `generateColdPlanForCycle` can find deliverables when
it runs.
...
- ✅ `deterministic.store.integration.test.js` - 17 tests (1.60s)
  - Includes: auto-deliverables seeding, determinism guarantees, constraint
    enforcement
