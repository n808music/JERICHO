...
Implemented a fully automatic deliverables and block generation system. After
goal admission, the system now:
...
**Result**: `suggestedBlocks.length > 0` for any reasonable goal with future
deadline and non-zero capacity.
...
- `buildAutoDeliverablesFromGoalContract(contract, nowDayKey, timeZone)` →
  Returns 3+ deliverables
- `detectCompoundGoal(contract)` → Identifies multi-outcome goals for rejection

**Logic:**

- **Music Release Detection**: Checks for keywords like "release", "album",
  "spotify", "tracks", "promo"
  - If detected: Generates music-specific phases (finalize masters → artwork →
    promo → daily execution)
  - If generic: 3-phase generic template (planning → execution → verification)
- **Block Scaling**: Calculates required blocks per deliverable based on days
  remaining
  - Short window (3-7 days): Fewer blocks per deliverable
  - Long window (30+ days): More granular breakdown across phases
- **Compound Goal Detection**: Pattern matching for conjunction keywords and
  semicolon-separated outcomes
...
```javascript
import {
  buildAutoDeliverablesFromGoalContract,
  detectCompoundGoal,
} from '../domain/autoStrategy.ts';
```
...
- User guidance: "Goal contains multiple outcomes; please choose one primary
...
- Stores metadata: `autoGenerated: true`,
  `autoStrategy: {detectedType, rationale}`

**New Feature: Auto-Run Plan Generation**

- After cycle + deliverables setup: Call
  `computeDerivedState(..., { type: 'GENERATE_COLD_PLAN' })`
...
- **Old**: If deliverables empty → Set error + return
- **New**: If deliverables empty → Auto-seed them, update strategy, continue
  planning
...
**Result**: Users never hit "NO_DELIVERABLES" error unless deadline is truly
invalid.
...
- Expanded detail view:
  - Green banner explaining: "Auto-generated deliverables · Edit optional ·
    derivable from goal commitment"
...
<details className="...">
  ...
  <div>
    {autoGenerated && (
      <div className="banner">Auto-generated deliverables...</div>
    )}
...
User submits contract
    ↓
detectCompoundGoal() ← POLICY ENFORCEMENT
    ├─ Is compound? → REJECT (MULTIPLE_OUTCOMES_DETECTED)
    └─ Single outcome? ↓
validateGoalAdmission() ← HARD GATES
    ├─ Failed validation? → REJECT
    └─ Passed? ↓
CREATE CYCLE
    ↓
AUTO-SEED DELIVERABLES
buildAutoDeliverablesFromGoalContract() ← NEW
    ├─ Detect type (music/generic)
    ├─ Scale blocks to time window
    ├─ Store as autoGenerated=true
    └─ Update cycle.strategy ↓
AUTO-GENERATE BLOCKS
computeDerivedState({type: 'GENERATE_COLD_PLAN'}) ← NEW
    └─ suggestedBlocks populated
            ↓
...
Call generateColdPlan() (coldPlan.ts)
    ├─ Allocates blocks to days
    ├─ Respects constraints
    └─ Returns ColdPlanV1
        ↓
Set lastPlanError if:
    ├─ 0 blocks generated, OR
...
- Suggests: "Choose one primary objective for this cycle; the other becomes an
  aspiration"
...
1. ✅ Returns >=3 deliverables for valid goal
2. ✅ Detects music release goals
3. ✅ Generates music-specific deliverables
4. ✅ Scales blocks based on time remaining
5. ✅ Handles invalid deadline gracefully
6. ✅ Detects compound goals with conjunction patterns

#### `src/domain/autoSeed.admission.test.ts` (3 tests)

1. ✅ Detects and rejects compound goals
2. ✅ Auto-seeds deliverables on admission
3. ✅ Triggers plan generation after admission
...
4. "How do I start planning?" ← Common question
...
✅ **After admission, `suggestedBlocks.length > 0`** for any sane goal with:

- Future deadline (not past, min 3 days away)
- Non-zero capacity (at least 1 block/day or block/week)
- Valid goal text

✅ **If blocks cannot be generated, UI shows specific error code + reasons**

- Example: "NO_BLOCKS_GENERATED: Deadline too soon; max blocks/day too low"

✅ **Deliverables UI remains editable, never required for planning**
...
- [ ] User feedback on auto-generated quality (👍/👎 to refine patterns)
...
- ✅ Backward compatible (existing cycles unaffected)
- ✅ No database migrations needed
- ✅ No breaking API changes
- ✅ Graceful fallback if auto-seed fails (still creates cycle)
- ✅ LocalStorage schema unchanged (metadata stored in cycle)
