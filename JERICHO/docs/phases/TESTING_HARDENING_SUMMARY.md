...
Fixed "long-running test" issue, hardened core generator against non-terminating
loops, and made test scripts deterministic.
...
while (current <= deadlineDayKey) {
  ...
  if (iterations > MAX_ITERATIONS) {
    throw new Error(
      `[deterministicPlanGenerator] getEligibleDays iteration cap exceeded...`
    );
  }
  // ... existing loop logic
}
...
- Prevents infinite loops if date arithmetic fails (e.g., same day returned
  repeatedly)
...
for (const block of blockQueue) {
  ...
  for (
    let attempt = 0;
    attempt < eligibleDays.length && !allocated;
    attempt++
  ) {
    ...
    if (allocationIterations > MAX_ALLOCATION_ITERATIONS) {
      return {
        ...
        error: {
          ...
          message:
            'Block allocation exceeded iteration limit (likely pathological constraints)',
        },
      };
    }
...
New test suite: "iteration guardrails: termination under pathological
constraints"
...
**Covers**: Immediate infeasibility detection, single-day iteration, zero
capacity.
...
**Covers**: Over-subscription detection, block queue exhaustion under tight
constraints.
...
3. `src/core/__tests__/deterministicPlanGenerator.test.ts` - 3 new regression
   tests
