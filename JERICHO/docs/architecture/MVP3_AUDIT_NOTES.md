...
**Purpose**: Document the authoritative state mutations for goal admission, plan
generation, execution, and cycle closure.
...
- `createBlock` has no gate for `goalAdmission.status === 'ADMITTED'` or cold
  plan existence.
...
- **Fix (PHASE 3)**: Add linkage status metadata; mark unlinked blocks; exclude
  from progress/convergence.
...
- **Fix (PHASE 2, 4)**: Implement `computeTerminalConvergence`, call at
  `endCycle`, store in `cycleSummary`.
...
- **Fix (PHASE 5)**: Add gate for
  `cycle.convergenceReport?.verdict === 'CONVERGED'`.
...
1. `endCycle`: call `computeTerminalConvergence`, store in
   `cycle.convergenceReport`
2. `computeProfileLearning`: filter by
   `cycle.convergenceReport?.verdict === 'CONVERGED'`
...
- Convergence computation must be pure:
  `(cycle, planProof, events, nowISO, timezone) → ConvergenceReport`.
