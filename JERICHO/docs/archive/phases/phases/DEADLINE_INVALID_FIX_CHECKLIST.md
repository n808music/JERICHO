...
- [x] Confirmed: `GoalExecutionContract.deadline.dayKey` is the canonical field
      (YYYY-MM-DD)
...
- [x] Implemented: `archiveAndCloneCycle()` function in identityCompute.js (~60
      lines)
...
3. `src/state/__tests__/planGeneration.deadlineValidation.test.ts` (8 tests, 250
   lines)
...
3. Monitor: Archive + Clone usage (should be minimal if deadline validation
   works)
...
**Regressions**: 0 **Breaking Changes**: 0
