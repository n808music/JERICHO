...
- Generate Plan currently dispatches `GENERATE_PLAN` and updates suggestions; no
  Apply action exists (Apply path to be added).

## Notes

- Evidence is currently completed execution events only (no planning churn).
- Feasibility gate runs in probability scorer; eligible/non-eligible computed
  separately.
